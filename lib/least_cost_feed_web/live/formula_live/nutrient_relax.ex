  ...
  def render(assigns) do
    ...
    <div class="w-3/4 min-w-[1000px] mx-auto p-5">
      ...
      <div :if={@status == :error} class="text-center py-20">
        <div class="text-xl font-bold text-error">{@error_msg}</div>
        <.link navigate={~p"/formulas/#{@formula.id}/edit"} class="btn btn-info  mt-4 inline-block">
          Back to Formula
        </.link>
      </div>
      ...
      <div :if={@status == :done}>
        ...
        <div :if={@suggestions == []} class="text-center py-10 text-base-content/50 text-lg">
        ...
        <div :if={@suggestions != []}>
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr :for={s <- @suggestions} class="border-b border-base-200 hover:bg-base-200">
        ...
        <div :if={@combined} class="mt-6">
          ...
          <table class="w-full text-sm">
            ...
            <tbody>
              <tr :for={s <- @combined.suggestions} class="border-b border-base-200">
                <td class="py-1 px-2">{s.nutrient_name} ({s.nutrient_unit})</td>
                <td class="py-1 px-2">{s.field}</td>
                <td class="py-1 px-2 text-right">{Helpers.float_decimal(s.current)}</td>
                <td class="py-1 px-2 text-right font-bold text-info">
        ...
        <div class="flex gap-2 mt-6">
          ...
          <.link navigate={~p"/formulas/#{@formula.id}/edit"} class="btn btn-neutral  w-[15%]">
            Cancel
          </.link>
        </div>
      </div>
